 ...
 *
 * Only two import lines in the app need to change:
 *   src/context/AppContext.jsx  →  from '../services/anthropicService.js'
 *   src/components/ChatDrawer.jsx → from '../services/anthropicService.js'
 *
 * SECURITY NOTE: this calls the Anthropic API directly from the browser using the
 * `anthropic-dangerous-direct-browser-access` header. Any VITE_* env var is baked
 * into the client bundle, so your API key is visible to anyone using the site.
 * That is acceptable for a personal/classroom demo IF you set a hard monthly spend
 * limit on the key in console.anthropic.com (Settings → Limits) and use a dedicated
 * key you can revoke. For a public deployment, put a tiny proxy in front instead.
 */
