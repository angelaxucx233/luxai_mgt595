...
export const lecture10Slides = [
  ...
  },
  {
    ...
    visualProps: {
      ...
      steps: [
        ...
        'Drag = turnover × cost per trade = 5 × 11 bp = 55 bp = 0.55% per year.',
    ...
    content: { eyebrow: 'Your turn', heading: 'Price a Strategy\u2019s Cost Drag', problemTitle: 'Your Turn: Price a Strategy\u2019s Cost Drag', footnote: 'Drag = turnover × cost per trade.' },
  },
  {
  ...
  },
  {
    ...
    content: {
      ...
      footnote: 'Discretionary → √size. Forced → linear. Crises are forced.',
    },
  },
  {
  ...
  // ── Block C: liquidity level and prices ──
  {
    ...
    content: {
      ...
      footnote: 'E(R) = r_f + risk premium + μ·c. Patience collects the last term.',
    },
  },
  {
    ...
    visualProps: {
      ...
      answers: [
        { label: 'μ×c premium (%/yr)', value: 2.0, tolerance: 0.02 },
        { label: 'AP total (%/yr)', value: 4.6, tolerance: 0.05 },
      ],
      steps: [
        'μ×c = 2 × 1% = 2.0% per year — the amortized cost of the marginal trader, capitalized into the price.',
    ...
    content: { eyebrow: 'Your turn', heading: 'Price the Illiquidity', problemTitle: 'Your Turn: Price the Illiquidity', footnote: 'Premium = μ×c · AP total = level + risk.' },
  },
  ...
  },
  {
    ...
    visualProps: {
      slideId: 12,
      scenario: 'For an emerging-market asset: cov(R,Rm) = 0.9, cov(c,cm) = 0.1, cov(R,cm) = −0.05 (it does badly when markets get illiquid), cov(c,Rm) = −0.15 (its costs spike when the market falls). Normalize var(Rm−cm) = 1, and take the net market premium E(Rm−cm−rf) = 6%.',
      ...
      steps: [
        'Net beta = β(R,Rm) + β(c,cm) − β(R,cm) − β(c,Rm) = 0.9 + 0.1 − (−0.05) − (−0.15) = 1.20.',
    ...
    content: { eyebrow: 'Your turn', heading: 'Compute a Net Beta', problemTitle: 'Your Turn: Compute a Net Beta', footnote: 'β_net = β(R,Rm) + β(c,cm) − β(R,cm) − β(c,Rm).' },
  },
  {
