...
const CLS = [
  { k: 'Currencies', f: 'C ∝ r_f* − r_f', d: 'F = S(1+r_f)/(1+r_f*): covered parity makes the futures discount equal the interest differential. Carry = the classic FX carry.', ex: 'AUD deposits at 7.09% funded by JPY at 0.87% → C ≈ 6.2%.' },
  ...
  { k: 'Commodities', f: 'C = δ − r_f', d: 'F = S(1 + r_f − δ): the convenience yield δ pulls futures below spot. Positive carry = backwardation; negative = contango.', ex: 'Oil in backwardation with δ = 6%, cash 2% → C = 4%.' },
  { k: 'Bonds', f: 'C ≈ (y_T − r_f) − D·(y_{T−1} − y_T)', d: 'Slope plus roll-down: you earn the curve above cash AND the price gain from aging into a lower yield. Slope trades = differences of two carries; Treasuries/credit get duration-adjusted (C/D).', ex: 'y₁₀ = 4%, cash 2%, D = 7, roll −0.1% → C = 2.0 + 0.7 = 2.7%.' },
  { k: 'Options', f: 'C ≈ [−θ + ν·(σ_{T−1} − σ_T)]/F', d: 'Theta decay plus the roll down the volatility term structure — an option\u2019s \u201cyield\u201d if the surface never moves.', ex: 'Selling rich short-dated puts harvests both terms — with tails attached.' },
];
  ...
  return (
    <div className="w-full max-w-lg flex flex-col gap-2.5">
      ...
      <div className="rounded-xl border border-yale-200 bg-white px-3 py-2.5">
        <div className="text-center rounded-lg bg-yale-900 text-white text-[13px] font-mono py-2">{c.f}</div>
