      ...
      <svg viewBox="0 0 400 152" className="w-full">
        ...
        {[1, 10, 100, 1000].map((v) => (
          <g key={v}>
            ...
            <text x={X(v)} y="145" textAnchor="middle" fontSize="8" fill="#94a3b8">{v}</text>
          </g>
        ))}
        {[2, 3, 4].map((v) => (
          <g key={v}>
            <text x="24" y={Y(v) + 3} textAnchor="end" fontSize="8" fill="#94a3b8">{v}</text>
            <line x1="30" y1={Y(v)} x2="392" y2={Y(v)} stroke="#e2e8f0" />
          </g>
        ))}
        <text x="36" y="16" fontSize="8.5" fill="#64748b">required t-stat (5% family-wise, Bonferroni)</text>
        <text x="388" y="145" textAnchor="end" fontSize="8" fill="#64748b">N tests (log)</text>
        {/* the old t=1.96 bar */}
        ...
        <text x={X(n)} y={Y(t) - 9} textAnchor="middle" fontSize="9.5" fill="#0f172a" fontWeight="800">t = {t.toFixed(2)}</text>
      </svg>
        ...
        {Array.from({ length: 10 }, (_, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={38 + i * 37} y="100" textAnchor="middle" fontSize="7.5" fill="#94a3b8">p({i + 1})</text>
            </g>
          );
        })}
        <text x="200" y="114" textAnchor="middle" fontSize="8.5" fill="#0f172a" fontWeight="700">{p.disc} of 10 declared discoveries</text>
      </svg>
      <div className="w-full rounded-xl bg-yale-50 border border-yale-100 px-3 py-2 text-[11px] text-yale-900">
  ...
  return (
    <div className="w-full max-w-lg flex flex-col gap-2.5">
      ...
      <svg viewBox="0 0 400 128" className="w-full">
        <text x="14" y="12" fontSize="8.5" fill="#64748b">random tests needed to produce one fluke at each t-stat (log scale)</text>
        <line x1="26" y1="104" x2="392" y2="104" stroke="#94a3b8" />
        {LUCK.map((x, i) => {
          const h = [18, 24, 40, 58, 84][i];
          return (
            <g key={x.t}>
              <rect x={40 + i * 72} y={104 - h} width="48" height={h} fill={i < 2 ? '#94a3b8' : i < 4 ? '#00356b' : '#d97706'} rx="4" />
              <text x={64 + i * 72} y={104 - h - 5} textAnchor="middle" fontSize="8" fill="#0f172a" fontWeight="800">{x.n}</text>
              <text x={64 + i * 72} y="117" textAnchor="middle" fontSize="8.5" fill="#64748b">t = {x.t}</text>
            </g>
          );
        })}
