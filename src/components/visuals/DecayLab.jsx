...
const MPBARS = [
  { l: 'in sample', sub: '323 months avg.', v: 0.582, c: '#3b82f6', note: 'The published number: mean predictor-portfolio return across the 97 studies, inside their own samples. Selected — by author and journal alike — on this very data.' },
      ...
      {tab === 'bars' ? (
        <>
          <svg viewBox="0 0 400 150" className="w-full">
            <text x="14" y="12" fontSize="8.5" fill="#a3b1c2">mean monthly portfolio return (%), 97 predictors — McLean &amp; Pontiff Table I</text>
            ...
            {MPBARS.map((x, i) => (
              <g key={x.l} onClick={() => setSel(i)} className="cursor-pointer">
                ...
                <text x={88 + i * 118} y="131" textAnchor="middle" fontSize="8" fill="#a3b1c2">{x.l}</text>
                <text x={88 + i * 118} y="141" textAnchor="middle" fontSize="7" fill="#cbd5e1">{x.sub}</text>
              </g>
            ))}
            <text x="147" y="26" fontSize="8.5" fill="#5eead4" fontWeight="700">−26%: statistical bias</text>
        ...
        <>
          <svg viewBox="0 0 400 156" className="w-full">
            <text x="14" y="12" fontSize="8.5" fill="#a3b1c2">predictor-portfolio coefficients — post-sample vs post-publication (Table VI)</text>
            ...
            {[
              ...
              <g key={r.l}>
                ...
                <text x={92 + i * 180} y={108 - r.a * 270 - 4} textAnchor="middle" fontSize="8.5" fill="#a3b1c2" fontWeight="700">{r.a.toFixed(3)}</text>
                <rect x={128 + i * 180} y={108 - r.b * 270} width="56" height={r.b * 270} fill="#e11d48" rx="3" />
                <text x={156 + i * 180} y={108 - r.b * 270 - 4} textAnchor="middle" fontSize="8.5" fill="#fb7185" fontWeight="800">{r.b.toFixed(3)}</text>
                <text x={124 + i * 180} y="121" textAnchor="middle" fontSize="8" fill="#a3b1c2">{r.l}</text>
              </g>
            ))}
            <text x="200" y="131" textAnchor="middle" fontSize="7.5" fill="#cbd5e1">grey = post-sample · red = post-publication · equality rejected at p = 0.000 · variance: unchanged</text>
...
const SEVEN = [
  ...
  { l: 'CAPM alphas', v: 82.4, c: '#3b82f6', note: 'Theory speaks to risk-ADJUSTED returns. A low-beta factor with negative raw return and positive alpha is BAB working, not failing. Still pure OLS — nothing Bayesian yet.' },
      ...
      {tab === 'bars' && (
        <>
          <svg viewBox="0 0 400 148" className="w-full">
            <text x="14" y="12" fontSize="8.5" fill="#a3b1c2">replication rate (%) — JKP Figure 1</text>
            ...
            {SEVEN.map((x, i) => (
              <g key={x.l} onClick={() => setSel(i)} className="cursor-pointer">
                ...
                <text x={52 + i * 51} y="124" textAnchor="middle" fontSize="6.4" fill="#a3b1c2">{x.l.split(' ')[0]}</text>
                <text x={52 + i * 51} y="132" textAnchor="middle" fontSize="6.4" fill="#cbd5e1">{x.l.split(' ').slice(1).join(' ')}</text>
              </g>
            ))}
          </svg>
      ...
      {tab === 'shrink' && (
        <>
          <svg viewBox="0 0 400 132" className="w-full">
            ...
            <text x="392" y="126" textAnchor="end" fontSize="8" fill="#a3b1c2">reported alpha α̂</text>
            <text x="14" y="16" fontSize="8" fill="#a3b1c2" transform="rotate(0)">E(α|α̂)</text>
            <line x1="36" y1="112" x2="380" y2="14" stroke="#e2e8f0" strokeWidth="1.4" strokeDasharray="4 3" />
            <text x="330" y="18" fontSize="7.5" fill="#cbd5e1">45°: believe the backtest</text>
            <line x1="36" y1="112" x2="380" y2={112 - kap * 98} stroke="#3b82f6" strokeWidth="2.4" />
            <text x="340" y={112 - kap * 98 + (kap > 0.6 ? 14 : -6)} fontSize="9" fill="#3b82f6" fontWeight="800">κ = {kap.toFixed(2)}</text>
      ...
      {tab === 'world' && (
        <>
          <svg viewBox="0 0 400 140" className="w-full">
            <text x="14" y="12" fontSize="8.5" fill="#a3b1c2">replication rate (%) by region — OLS vs BY vs Bayes-all-data</text>
            <line x1="26" y1="112" x2="392" y2="112" stroke="#94a3b8" />
            {[
              ...
              <g key={r.l}>
                ...
                <text x={88 + i * 124} y="124" textAnchor="middle" fontSize="7.5" fill="#a3b1c2">{r.l}</text>
              </g>
            ))}
            <text x="200" y="137" textAnchor="middle" fontSize="7.5" fill="#cbd5e1">grey OLS · red Benjamini–Yekutieli · green empirical Bayes (all global data)</text>
          </svg>
...
const CW = [
  { l: 'Through 2005 · all stocks', v: 48, pos: 99, x: 0, y: 0, c: '#3b82f6', note: 'The published environment: median anomaly earns 48 bp/month and 99% of anomalies are positive. This is the world the papers describe.' },
      ...
      {tab === 'grid' ? (
        <>
          <svg viewBox="0 0 400 158" className="w-full">
            <text x="132" y="14" textAnchor="middle" fontSize="8.5" fill="#a3b1c2" fontWeight="700">all stocks</text>
            <text x="298" y="14" textAnchor="middle" fontSize="8.5" fill="#a3b1c2" fontWeight="700">top 90% mkt cap</text>
            <text x="48" y="52" textAnchor="middle" fontSize="8.5" fill="#a3b1c2" fontWeight="700">thru 2005</text>
            <text x="48" y="118" textAnchor="middle" fontSize="8.5" fill="#a3b1c2" fontWeight="700">post 2005</text>
            {CW.map((q, i) => (
            ...
            <text x="298" y="152" textAnchor="middle" fontSize="8" fill="#fb7185" fontWeight="700">together: −85%</text>
          </svg>
          <div className="w-full rounded-xl bg-yale-50 border border-yale-100 px-3 py-2 text-[11px] text-yale-900 min-h-[58px]">
            <b>{c.l}:</b> {c.note}
          </div>
        ...
        <>
          <div className="w-full grid grid-cols-2 gap-1.5 text-[10.5px]">
            {[
              ['Harvey–Liu–Zhu', 'Much of the zoo is multiple-testing NOISE: at 316 factors, t = 2 is the expected best draw. 132–158 of 296 false.', '#3b82f6'],
