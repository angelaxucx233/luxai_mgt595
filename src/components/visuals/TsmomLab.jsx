...
const TCOL = { Equities: '#00356b', Bonds: '#0f766e', Currencies: '#7c3aed', Commodities: '#d97706' };
      ...
      <svg viewBox="0 0 400 158" className="w-full">
        ...
        {rows.map(([l, v], i) => (
          <g key={l}>
            ...
            <text x={28 + i * (bw + 4) + bw / 2} y="140" fontSize="6" fill="#64748b" transform={`rotate(-38 ${28 + i * (bw + 4) + bw / 2} 140)`} textAnchor="end">{l}</text>
          </g>
        ))}
      </svg>
      ...
      <svg viewBox="0 0 400 168" className="w-full">
        ...
        {DD.map((x, i) => (
          <g key={x.l} onClick={() => setSel(i)} className="cursor-pointer" opacity={sel === i ? 1 : 0.55}>
            ...
            <rect x={42 + i * 37} y={x.t >= 0 ? 92 - x.t * 0.72 : 92} width="14" height={Math.abs(x.t) * 0.72} fill={x.t >= 0 ? '#00356b' : '#e11d48'} rx="2" />
            <text x={41 + i * 37} y={x.t >= 0 ? 92 - x.t * 0.72 - 3 : 92 - 3} textAnchor="middle" fontSize="6.3" fill={x.t >= 0 ? '#00356b' : '#e11d48'} fontWeight="800">{x.t > 0 ? '+' : ''}{x.t.toFixed(0)}</text>
            <text x={41 + i * 37} y={92 - x.p * 0.72 + 8} textAnchor="middle" fontSize="6.3" fill="#64748b">{x.p.toFixed(0)}</text>
          </g>
        ))}
        <text x="200" y="164" textAnchor="middle" fontSize="8.5" fill="#059669" fontWeight="700">positive in 8 of the 10 worst episodes a balanced portfolio has ever had</text>
      </svg>
      ...
      {tab === 'impulse' ? (
        <>
          <svg viewBox="0 0 400 132" className="w-full">
            ...
            <text x="120" y="30" fontSize="8.5" fill="#0f766e" fontWeight="700">continuation ≈ 12 months</text>
            <text x="300" y="30" fontSize="8.5" fill="#e11d48" fontWeight="700">partial reversal</text>
            <text x="210" y="118" textAnchor="middle" fontSize="8" fill="#94a3b8">~12 mo</text>
            <text x="120" y="98" fontSize="8" fill="#64748b">under-reaction builds the trend…</text>
        ...
        <>
          <svg viewBox="0 0 400 112" className="w-full">
            <rect x="30" y="26" width="150" height="60" rx="9" fill="#00356b" />
