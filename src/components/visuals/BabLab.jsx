  ...
  return (
    <div className="w-full max-w-lg flex flex-col items-center gap-2.5">
      <svg viewBox="0 0 400 150" className="w-full">
        ...
        <text x="386" y="140" textAnchor="end" fontSize="8.5" fill="#64748b">β</text>
        <text x="14" y="14" fontSize="8.5" fill="#64748b">E[r]%</text>
        {[0.5, 1, 1.5, 2].map((b) => (
          <text key={b} x={X(b)} y="140" textAnchor="middle" fontSize="8" fill="#94a3b8">{b}</text>
        ))}
        {/* CAPM line */}
        <line x1={X(0)} y1={Y(capm(0))} x2={X(2)} y2={Y(capm(2))} stroke="#94a3b8" strokeWidth="1.6" strokeDasharray="5 4" />
        <text x={X(1.72)} y={Y(capm(1.72)) - 6} fontSize="8" fill="#94a3b8">CAPM</text>
        {/* funding line */}
        <line x1={X(0)} y1={Y(fund(0))} x2={X(2)} y2={Y(fund(2))} stroke="#00356b" strokeWidth="2.4" />
        <text x={X(1.62)} y={Y(fund(1.62)) + 13} fontSize="8.5" fill="#00356b" fontWeight="700">funding CAPM</text>
        {/* pivot at beta=1 */}
        <circle cx={X(1)} cy={Y(capm(1))} r="3.5" fill="#0f766e" />
        <text x={X(1) + 6} y={Y(capm(1)) - 6} fontSize="8" fill="#0f766e" fontWeight="700">pivot: β = 1</text>
        {/* alpha wedge at beta 0.5 */}
        <line x1={X(0.5)} y1={Y(capm(0.5))} x2={X(0.5)} y2={Y(fund(0.5))} stroke="#059669" strokeWidth="2.4" />
        <text x={X(0.5) - 5} y={(Y(capm(0.5)) + Y(fund(0.5))) / 2 + 3} textAnchor="end" fontSize="8.5" fill="#059669" fontWeight="800">+α</text>
        {/* alpha wedge at beta 1.5 */}
        <line x1={X(1.5)} y1={Y(capm(1.5))} x2={X(1.5)} y2={Y(fund(1.5))} stroke="#e11d48" strokeWidth="2.4" />
        <text x={X(1.5) + 5} y={(Y(capm(1.5)) + Y(fund(1.5))) / 2 + 3} fontSize="8.5" fill="#e11d48" fontWeight="800">−α</text>
      </svg>
        ...
        <>
          <svg viewBox="0 0 400 140" className="w-full">
            ...
            {[
              ...
              { l: 'UMD', v: 0.50, c: '#94a3b8' }, { l: 'BAB', v: 0.75, c: '#00356b' },
            ].map((b, i) => (
              <g key={b.l}>
                ...
                <text x={78 + i * 88} y="130" textAnchor="middle" fontSize="8.5" fill="#64748b">{b.l}</text>
              </g>
            ))}
          </svg>
          <div className="w-full rounded-xl bg-yale-50 border border-yale-100 px-3 py-2 text-[11px] text-yale-900">
      ...
      {tab === 'ted' && (
        <>
          <svg viewBox="0 0 400 128" className="w-full">
            <text x="14" y="12" fontSize="8.5" fill="#64748b">3-yr BAB return (navy) vs 3-yr TED spread, inverted (amber) — stylized</text>
            <line x1="20" y1="70" x2="392" y2="70" stroke="#e2e8f0" />
            <path d="M 24 44 C 70 32, 110 60, 150 50 C 200 38, 240 92, 285 98 C 320 102, 360 52, 388 40" fill="none" stroke="#00356b" strokeWidth="2.2" />
      ...
      {tab === 'holders' && (
        <>
          <svg viewBox="0 0 400 118" className="w-full">
            <text x="14" y="12" fontSize="8.5" fill="#64748b">average beta of holdings</text>
            <line x1="200" y1="24" x2="200" y2="100" stroke="#e2e8f0" />
            <text x="200" y="112" textAnchor="middle" fontSize="8" fill="#94a3b8">β = 1</text>
            {[
              { l: 'Mutual funds', b: 1.12, y: 34 }, { l: 'Individuals', b: 1.08, y: 54 },
              { l: 'LBO deals', b: 0.85, y: 74 }, { l: 'Berkshire', b: 0.77, y: 94 },
            ].map((r) => (
              <g key={r.l}>
                <text x="14" y={r.y + 3} fontSize="8.5" fill="#334155" fontWeight="600">{r.l}</text>
