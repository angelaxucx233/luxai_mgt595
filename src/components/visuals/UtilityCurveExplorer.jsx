  ...
  return (
    <div className="w-full max-w-lg flex flex-col items-center gap-3">
      <svg viewBox="0 0 420 240" className="w-full">
        ...
        <path d={curve} fill="none" stroke="#3b82f6" strokeWidth="2.5" />
        {/* chord */}
        <line x1={X(wl)} y1={Y(uL)} x2={X(wh)} y2={Y(uH)} stroke="#b45309" strokeWidth="1.5" strokeDasharray="5 4" />
        <circle cx={X(wl)} cy={Y(uL)} r="4" fill="#3b82f6" />
        <circle cx={X(wh)} cy={Y(uH)} r="4" fill="#3b82f6" />
        ...
        <circle cx={X(mean)} cy={Y(uMean)} r="5" fill="#d97706" stroke="#3b82f6" strokeWidth="1" />
        {/* pi bracket */}
        <line x1={X(ce)} y1="222" x2={X(mean)} y2="222" stroke="#b45309" strokeWidth="2" />
        <text x={(X(ce) + X(mean)) / 2} y="236" textAnchor="middle" fontSize="11" fill="#f59e0b" fontWeight="700">
          π = {pi.toFixed(1)}
        </text>
        <text x={X(wl)} y="225" textAnchor="middle" fontSize="10" fill="#a3b1c2">W_L</text>
        <text x={X(wh)} y="225" textAnchor="middle" fontSize="10" fill="#a3b1c2">W_H</text>
        <text x={X(mean) + 4} y={Y(uMean) - 8} fontSize="10" fill="#d97706" fontWeight="600">u(E[W])</text>
        <text x={X(mean) + 8} y={Y(eU) + 12} fontSize="10" fill="#fb7185" fontWeight="600">E[u(W)]</text>
        <text x={X(ce)} y="207" textAnchor="end" fontSize="10" fill="#fb7185">CE</text>
        <text x="398" y="224" textAnchor="end" fontSize="10" fill="#a3b1c2">wealth W</text>
        <text x="36" y="14" textAnchor="end" fontSize="10" fill="#a3b1c2">u(W)</text>
      </svg>
